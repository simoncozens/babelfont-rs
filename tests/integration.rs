...
fn test_load_babelfont() -> Result<(), BabelfontError> {
    ...
    let wght_axis = font.axes.iter().find(|ax| ax.tag == Tag::new(b"wght")).unwrap();
    ...
    assert_eq!(
        first_master.name.get_default().unwrap(),
        "Regular"
    );
    ...
    assert_eq!(
        first_instance.name.get_default().unwrap(),
        "Regular"
    );
